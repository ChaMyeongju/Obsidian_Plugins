...
export default class ConfigSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      .setName("Better Export PDF Auto Save")
      .setDesc("Maintained by Myeongju Cha. This plugin is a fork of l1xnan/obsidian-better-export-pdf.");
    ...
    new Setting(containerEl)
      .setName(this.i18n.settings.autoSaveNextToMarkdown)
      .setDesc("Skip the save dialog for single note export and save the PDF next to the source markdown file")
      .addToggle((cb) => {
        cb.setValue(this.plugin.settings.autoSaveNextToMarkdown).onChange(async (value) => {
          this.plugin.settings.autoSaveNextToMarkdown = value;
          await this.plugin.saveSettings();
        });
      });
