<script lang="ts">
  ...
  import { exportToPDF, getOutputFile, getOutputPath, getPdfPathNextToMarkdown, toVaultRelativePath } from "../pdf";
  ...
  export async function handlePrintToPDF() {
    ...
    const autoSaveNextToMarkdown = modal.options.autoSaveNextToMarkdown;
        ...
        if (outputPath) {
          await Promise.all(
            webviews.map(async (wb, i) => {
              await exportToPDF(`${outputPath}/${docs[i].file.basename}.pdf`, { ...settings, ...config }, wb, docs[i], true);
            }),
          );
        }
      } else {
        const outputFile = autoSaveNextToMarkdown
          ? getPdfPathNextToMarkdown(modal.app, docs[0].file, settings.isTimestamp)
          : await getOutputFile(title, settings.isTimestamp);
        if (outputFile) {
          await exportToPDF(outputFile, { ...settings, ...config }, webviews[0], docs[0], true);
          if (autoSaveNextToMarkdown) {
            new Notice(`PDF exported: ${toVaultRelativePath(modal.app, outputFile)}`);
          }
        }
      }
    } catch (error) {
