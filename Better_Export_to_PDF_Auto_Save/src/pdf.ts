...
import { App, FileSystemAdapter, TFile, type FrontMatterCache } from "obsidian";
...
export function getVaultBasePath(app: App): string {
  ...
  throw new Error("Cannot resolve vault base path. This plugin only works on desktop vaults.");
}

export function getPdfPathNextToMarkdown(app: App, file: TFile, isTimestamp?: boolean): string {
  const basePath = getVaultBasePath(app);
  const mdDir = path.dirname(file.path);
  const suffix = isTimestamp ? `-${Date.now()}` : "";

  return path.join(basePath, mdDir, `${file.basename}${suffix}.pdf`);
}

export function toVaultRelativePath(app: App, filePath: string): string {
  const basePath = getVaultBasePath(app);
